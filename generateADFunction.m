function [] = generateADFunction(pathOpenSimModel, outputDir,...
    outputFilename, options, opts)
% --------------------------------------------------------------------------
% generateADFunction
...
arguments
% required inputs
    pathOpenSimModel (1,:) char {mustBeFile}
    outputDir (1,:) char
...
% options as name-value pairs
    opts.jointsOrder (1,:) cell = {};
    opts.coordinatesOrder (1,:) cell = {};

    opts.input3DBodyForces = [];
    opts.input3DBodyMoments = [];

    opts.export3DPositions = [];
    opts.export3DOrientations = [];
    opts.export3DVelocities = [];
    opts.export3DVelocitiesProjGround = [];
    opts.exportGRFs (1,1) logical = false;
...
% combine options (struct) and opts (name-value pairs) into a single struct
for s=string(fieldnames(options)')
    if isfield(opts, s)
        class_opts = class(opts.(s));
        if isa(options.(s), class_opts)
            opts.(s) = options.(s);
        else
            warning("Option '%s' ignored. Expected a %s, but got a %s.",...
                class_opts,class(options.(s)))
        end
    else
        warning("Unknown option '%s' ignored.",s)
    end
end
