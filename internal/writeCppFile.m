...
%   Example input:
%     options.input3DBodyForces(1).body = 'torso';
%     options.input3DBodyForces(1).point_in_body = [-0.1, 0.3, 0];
%     options.input3DBodyForces(1).name = 'back_push';
%     options.input3DBodyForces(1).reference_frame = 'ground';
% 
%   - options.input3DBodyMoments -
%   * define inputs to add moments acting on bodies. Moments are expressed as 
%   [x, y, z] components in given reference frame. [array of structs] 
%   Example input:
%     options.input3DBodyMoments(1).body = 'tibia_l';
%     options.input3DBodyMoments(1).name = 'exo_shank_l';
%     options.input3DBodyMoments(1).reference_frame = 'tibia_l';
%     options.input3DBodyMoments(2).body = 'calcn_l';
%     options.input3DBodyMoments(2).name = 'exo_foot_l';
%     options.input3DBodyMoments(2).reference_frame = 'tibia_l';
%
%   - options.export3DPositions
%   * points of which the position in ground frame should be exported. 
%   [array of structs] Example input:
%       options.export3DPositions(1).body = 'tibia_l';
%       options.export3DPositions(1).point_in_body = [0, -0.012, 0];
%       options.export3DPositions(1).name = 'left_shin';
%       options.export3DPositions(2).body = 'tibia_r';
%       options.export3DPositions(2).point_in_body = [0, -0.012, 0];
%       options.export3DPositions(2).name = 'right_shin';
%
%   - options.export3DOrientations
%   * Export the relative roation between 2 bodies as a quaternion 
%   [array of structs] Example input:
%       options.export3DOrientations(1).body = 'calcn_r';
%       options.export3DOrientations(1).reference_frame = 'tibia_r';
%       options.export3DOrientations(1).name = 'ankle_quat';
%
%   - options.export3DVelocities
%   * points of which the velocity in ground frame should be exported. 
%   [array of structs] Example input:
%       options.export3DVelocities(1).body = 'tibia_l';
%       options.export3DVelocities(1).point_in_body = [0, -0.012, 0];
%       options.export3DVelocities(1).name = 'left_shin';
%       options.export3DVelocities(2).body = 'tibia_r';
%       options.export3DVelocities(2).point_in_body = [0, -0.012, 0];
%       options.export3DVelocities(2).name = 'right_shin';
%
%   - options.exportGRFs
...
if isempty(options.jointsOrder) || isempty(options.coordinatesOrder)
    options.jointsOrder = {};
    options.coordinatesOrder = {};
    for i = 0:nJoints-1
        joint_i = jointSet.get(i);
        joint_name_i = char(joint_i.getName());
        if ~contains(joint_name_i, 'patel') % do not add patella joint to options.jointsOrder
            options.jointsOrder{end+1} = joint_name_i;
            ...
            for j = 0:joint_i_Nc-1
                ...
                if joint_i_coor.get_locked()
                    ...
                    options.coordinatesOrder{end+1} = char(joint_i_coor.getName());
                end
            end
        end
    end
...
if ~isempty(options.input3DBodyForces)
    nInputsControls = nInputsControls + 3*length(options.input3DBodyForces);
end
if ~isempty(options.input3DBodyMoments)
    nInputsControls = nInputsControls + 3*length(options.input3DBodyMoments);
end

% Total number of outputs for the external function
nOutputs = nCoordinates; % ID moment/force per coordinate
if options.exportGRFs
    nOutputs = nOutputs + 6; % 3 right and 3 left
end
if options.exportSeparateGRFs
    nOutputs = nOutputs + 3*nContacts;
end
if options.exportContactPowers
    nOutputs = nOutputs + nContacts;
end
if options.exportGRMs
    nOutputs = nOutputs + 6; % 3 right and 3 left
end
if ~isempty(options.export3DPositions)
    nOutputs = nOutputs + 3*length(options.export3DPositions);
end
if ~isempty(options.export3DOrientations)
    nOutputs = nOutputs + 4*length(options.export3DOrientations);
end
if ~isempty(options.export3DVelocities)
    nOutputs = nOutputs + 3*length(options.export3DVelocities);
end
if ~isempty(options.export3DVelocitiesProjGround)
    nOutputs = nOutputs + 3*length(options.export3DVelocitiesProjGround);
end
...
% if ispc
%     fprintf(fid, 'SimTK::Array_<int> getIndicesOpenSimInSimbody(const Model& model) { \n');
...
%     fprintf(fid, '} \n\n');
%     
%     fprintf(fid, 'SimTK::Array_<int> getIndicesSimbodyInOpenSim(const Model& model) { \n');
%     fprintf(fid, '\tauto idxOSInSimbody = getIndicesOpenSimInSimbody(model); \n');
%     fprintf(fid, '\tauto s = model.getWorkingState(); \n');
...
if ~isempty(options.jointsOrder)
    for i = 1:length(options.jointsOrder)
        jointOrder = options.jointsOrder{i};
        fprintf(fid, '\tmodel->addJoint(%s);\n', jointOrder);
        coordi = [];
        
        try
            c_joint = jointSet.get(jointOrder); % will fail if joint from options.jointsOrder is not in jointSet (i.e. model file)
            c_joint_name = char(c_joint.getName());
    ...
    assert(length(options.jointsOrder) == nJoints, 'options.jointsOrder and jointSet have different sizes');
end
...
for i=1:length(options.input3DBodyForces)
    if i==1
        fprintf(fid, '\t/// forces acting on bodies\n');
    end
    fprintf(fid, '\tVec3 Point_%s = Vec3(%.20f, %.20f, %.20f);\n', options.input3DBodyForces(i).name, options.input3DBodyForces(i).point_in_body(1), options.input3DBodyForces(i).point_in_body(2), options.input3DBodyForces(i).point_in_body(3));
    fprintf(fid, '\tVec3 Force_%s;', options.input3DBodyForces(i).name);
    fprintf(fid, '\tfor (int i = 0; i < 3; ++i) Force_%s[i] = u[%i+i];\n', options.input3DBodyForces(i).name, countInputU);
    
    if strcmpi(options.input3DBodyForces(i).reference_frame,'ground')
        fprintf(fid, '\tVec3 Force_%s_inG = Force_%s;\n', options.input3DBodyForces(i).name, options.input3DBodyForces(i).name);
    else
        fprintf(fid, '\tVec3 Force_%s_inG = %s->getMobilizedBody().expressVectorInGroundFrame(*state, Force_%s);\n', options.input3DBodyForces(i).name, options.input3DBodyForces(i).reference_frame, options.input3DBodyForces(i).name);
    end

    fprintf(fid,'\tmodel->getMatterSubsystem().addInStationForce(*state, %s->getMobilizedBodyIndex(), Point_%s, Force_%s_inG, appliedBodyForces);\n\n', options.input3DBodyForces(i).body, options.input3DBodyForces(i).name, options.input3DBodyForces(i).name);

    IO_indices.input.Forces.(options.input3DBodyForces(i).name) = countInputU + [1:3] + 2*nCoordinates;
    countInputU = countInputU + 3;
end

% Add input moments
for i=1:length(options.input3DBodyMoments)
    if i==1
        fprintf(fid, '\t/// moments acting on bodies\n');
    end
    fprintf(fid, '\tVec3 Moment_%s;\n', options.input3DBodyMoments(i).name);
    fprintf(fid, '\tfor (int i = 0; i < 3; ++i) Moment_%s[i] = u[%i+i];\n', options.input3DBodyMoments(i).name, countInputU);
    
    if strcmpi(options.input3DBodyMoments(i).reference_frame,'ground')
        fprintf(fid, '\tVec3 Moment_%s_inG = Moment_%s;\n', options.input3DBodyMoments(i).name, options.input3DBodyMoments(i).name);
    else
        fprintf(fid, '\tVec3 Moment_%s_inG = %s->getMobilizedBody().expressVectorInGroundFrame(*state, Moment_%s);\n', options.input3DBodyMoments(i).name, options.input3DBodyMoments(i).reference_frame, options.input3DBodyMoments(i).name);
    end

    fprintf(fid,'\tmodel->getMatterSubsystem().addInBodyTorque(*state, %s->getMobilizedBodyIndex(), Moment_%s_inG, appliedBodyForces);\n\n', options.input3DBodyMoments(i).body, options.input3DBodyMoments(i).name);

    IO_indices.input.Moments.(options.input3DBodyMoments(i).name) = countInputU + [1:3] + 2*nCoordinates;
    countInputU = countInputU + 3;
end
...
if ~isempty(options.export3DPositions)
    ...
    for i = 1:length(options.export3DPositions)
        segment = options.export3DPositions(i).body;
        station = options.export3DPositions(i).point_in_body;
        name = options.export3DPositions(i).name;
...
if ~isempty(options.export3DOrientations)
    ...
    for i = 1:length(options.export3DOrientations)
        fprintf(fid, '\tQuaternion %s_orientation = %s->getMobilizedBody().findBodyRotationInAnotherBody(*state, %s->getMobilizedBody()).convertRotationToQuaternion();\n',...
            options.export3DOrientations(i).name, options.export3DOrientations(i).body, options.export3DOrientations(i).reference_frame);
    end
    fprintf(fid, '\n');
end
...
if ~isempty(options.export3DVelocities)
    ...
    for i = 1:length(options.export3DVelocities)
        segment = options.export3DVelocities(i).body;
        station = options.export3DVelocities(i).point_in_body;
        name = options.export3DVelocities(i).name;
...
if ~isempty(options.export3DVelocitiesProjGround)
    ...
    for i = 1:length(options.export3DVelocitiesProjGround)
        segment = options.export3DVelocitiesProjGround(i).body;
        station = options.export3DVelocitiesProjGround(i).point_in_body;
        name = options.export3DVelocitiesProjGround(i).name;
...
nInputsF = 2*nCoordinates + countInputU;
IO_indices.input.nInputs = nInputsF;
% IO_indices.options.coordinatesOrder = options.coordinatesOrder;

% positions
if ~isempty(options.export3DPositions)
    IO_point_pos = struct();
    for c_seg = 1:length(options.export3DPositions)
        name = options.export3DPositions(c_seg).name;
    ...
    count_acc = count_acc + 3 * length(options.export3DPositions);
...
if ~isempty(options.export3DOrientations)
    ...
    for c_seg = 1:length(options.export3DOrientations)
        name = options.export3DOrientations(c_seg).name;
    ...
    count_acc = count_acc + 4 * length(options.export3DOrientations);
...
if ~isempty(options.export3DVelocities)
    IO_point_vel = struct();
    for c_seg = 1:length(options.export3DVelocities)
        name = options.export3DVelocities(c_seg).name;
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(%s_velInGround[i]);\n', count_acc + (c_seg-1) * 3, name);
        tmp = outputCount + count_acc + (c_seg - 1) * 3;
        segment_i = tmp : tmp + 2;
        IO_point_vel.(name) = segment_i;
    end
    count_acc = count_acc + 3 * length(options.export3DVelocities);
...
if ~isempty(options.export3DVelocitiesProjGround)
    ...
    for c_seg = 1:length(options.export3DVelocitiesProjGround)
        name = options.export3DVelocitiesProjGround(c_seg).name;
        fprintf(fid, '\tfor (int i = 0; i < 3; ++i) res[0][i + nCoordinates + %i] = value<T>(%s_velProjOnGround[i]);\n', count_acc + (c_seg-1) * 3, name);
        tmp = outputCount + count_acc + (c_seg - 1) * 3;
        segment_i = tmp : tmp + 2;
        IO_point_projvel.(name) = segment_i;
    end
    count_acc = count_acc + 3 * length(options.export3DVelocitiesProjGround);
...
if (options.exportGRFs || options.exportSeparateGRFs)
...
IO = IO_indices;
save([pathOutputFile,'_IO.mat'],'IO')

%% Main function
fprintf(fid, 'int main() {\n');
fprintf(fid, '\tRecorder::start_recording("%s");\n', replace(pathRecorderStream,'\','/'));
