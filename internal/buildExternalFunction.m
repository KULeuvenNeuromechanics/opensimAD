...
path_external_functions_filename_install = fullfile(pathMain,...
    'installExternalFunction', outputFilename);
mkdir(path_external_functions_filename_install);
path_external_functions_build = fullfile(pathMain, 'buildExternalFunction');
path_external_functions_filename_build = fullfile(pathMain,...
    'buildExternalFunction', outputFilename);
mkdir(path_external_functions_filename_build);

%% use cmake to compile foo_jac.c to .dll
copyfile(fullfile(path_external_functions_build,'CMakeLists.txt'),...
...
if ispc
    cmd1 = ['cmake "', path_external_functions_filename_build, '" -G "',...
        compiler, '" -DTARGET_NAME:STRING="', outputFilename,...
        '" -DINSTALL_DIR:PATH="', path_external_functions_filename_install, '"'];
    cmd2 = 'cmake --build . --config RelWithDebInfo --target install';

elseif isunix
    cmd1 = ['cmake "', path_external_functions_filename_build,...
